...
@implementation BuildAPI


@synthesize devices, models, accounts, errorMessage, statusMessage, deviceCode, agentCode;
...
{
	// As getModels, but this time with an optional 'getDevice' parameter:
	// YES: automat
...
- (void)restartDevices:(NSString *)modelID {
    ...
    if (modelID == nil || modelID.length == 0) {
		...
		errorMessage = @"[ERROR] Could not create a request to reastart the app’s device: invalid model ID.";
        [self reportError];
        return;
    }
	...
	if (request) {
		...
		errorMessage = @"[ERROR] Could not create a request to restart the model’s devices.";
		[self reportError];
	}
}
...
- (void)autoRenameDevice:(NSString *)deviceID {
	...
	// This method should be used SOLELY to change the name of new, unassigned devices
...
- (void)processAccounts {
	...
	// If we don't have a 'devices' count, bail
	if (devices.count < 1) return;
	
	NSString *uuid;
	
	for (NSUInteger i = 0 ; i < devices.count ; ++i) {
		NSDictionary *aDev = [devices objectAtIndex:i];
		
		// Check for owning accounts
		uuid = [aDev objectForKey:@"owned_by"];
		BOOL gotFlag = NO;
		
		for (NSUInteger j = 0 ; j < accounts.count ; ++j) {
			NSString *acct = [accounts objectAtIndex:j];
			if ([acct compare:uuid] == NSOrderedSame) gotFlag = YES;
		}
		
		// Check for creator accounts		
		uuid = [aDev objectForKey:@"created_by"];
		for (NSUInteger j = 0 ; j < accounts.count ; ++j) {
			NSString *acct = [accounts objectAtIndex:j];
			if ([acct compare:uuid] == NSOrderedSame) gotFlag = YES;
		}
		...
		if (!gotFlag)  {
			// This UUID is new, so add it to the list
			[accounts addObject:uuid];
		}
	}
	
	for (NSUInteger i = 0 ; i < accounts.count ; ++i) {
		//NSLog([accounts objectAtIndex:i]);
	}
}
...
- (void)startLogging {
    ...
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:_logURL] cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:kStreamTimeout];
...
- (void)launchConnection:(id)request :(NSInteger)actionCode {
	...
	if (actionCode == kConnectTypeGetLogEntriesStreamed) [req setTimeoutInterval:kStreamTimeout];
	...
	if (_connexions.count == 0) {
        ...
        // and then add the new connexion to the list of current connections. This is the ONLY connexion
            ...
            switch (connexion.actionCode) {
                ...
                case kConnectTypeGetDevices: {
                    ...
                    for (NSDictionary *device in devs) {
                        ...
                        // id - string
                        // name - string
                        // powerstate - string: "online" or "offline"
                        // rssi - integer
                        // agent_id - string
                        // agent_status - string: "online" or "offline"
                        // model_id - string
						// owned_by - string: UUID indicating owning account
						// created_by - string: UUID indicating creator (ie. BlinkUp) account
						...
						// Deal with *unexpected* null values ahead of time

						NSString *deviceState = [newDevice valueForKey:@"powerstate"];
						if ((NSNull *)deviceState == [NSNull null])
						{
							// 'powerstate' is null for some unexpected reason (agent offline?)
						...
						if ((NSNull *)deviceState == [NSNull null])
						{
							// 'agent_status' is null for some unexpected reason (agent offline?)
